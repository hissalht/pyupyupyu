...
caractéristiques. Tout d'abord, la taille du ``vaisseau'' du joueur se limite
dans la plupart des Manic Shooter à un pixel, pourquoi ? Tout simplement pour
pouvoir faire face à la deuxième caractèristique phare du Manic Shooter, la
multitude de projectiles qui apparaissent à l'écran comme vous pouvez le voir
sur cette image tirée de Touhou Project
...
Notre jeu est codé sous Python 3 et nous utilisons pysdl2 qui est une librairie
de Python pour gérer tout ce qui concerne le multimédia pour notre jeu, à savoir
l'ambiance sonore et les graphismes.

Nous avons également des fichiers à part, sous l'extension .dcbbf qui sont les
fichiers de blocks du jeu.
